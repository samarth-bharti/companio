...
export default async function AdminSurge() {
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {/* Create */}
      <form action={createSurge} className="rounded-2xl bg-white border border-[var(--color-ink)]/10 p-4 grid grid-cols-2 md:grid-cols-5 gap-3 items-end">
        ...
        <button className="h-10 col-span-2 md:col-span-5 md:w-40 rounded-full bg-[var(--color-azure)] text-white text-sm font-semibold">Add surge window</button>
      </form>
      ...
      <div className="flex flex-col gap-3">
        ...
        {periods.map((p) => (
          <div key={p.id} className="rounded-2xl bg-white border border-[var(--color-ink)]/10 p-4 flex items-center gap-4 flex-wrap">
            ...
            <form action={toggleSurge}>
              <input type="hidden" name="id" value={p.id} />
              <input type="hidden" name="active" value={String(p.active)} />
              <button className="text-xs font-semibold px-3 py-1.5 rounded-full border border-[var(--color-ink)]/20 text-[var(--color-ink)]">
